...
class KnowledgeDatabase(object):
    ...
    def _create_terms(self, path):
        ...
        self.terms = []
        for index, row in data.iterrows():
            self.terms.append(Term(term_id=row["term_id"],
                                   term=row["term"],
    ...
    def _create_reasons(self, path):
        ...
        self.reasons = []
        for index, row in data.iterrows():
            self.reasons.append(Reason(reason_id=row["reason_id"],
    ...
    def _create_results(self, path):
        ...
        self.results = []
        for index, row in data.iterrows():
            self.results.append(Result(result_id=row["reason_id"],
                                       event_id=row["event_id"],
                                       description=row["description"]))
    ...
    def _create_tasks_date(self, path):
        ...
        self.tasks_date = []
        for index, row in data.iterrows():
            self.tasks_date.append(TasksDate(task_id=row["task_id"],
                                             event_id=row["event_id"],
                                             level=row["level"],
                                             question=["question"]))
    ...
    def _create_tasks_event(self, path):
        ...
        self.tasks_event = []
        for index, row in data.iterrows():
            self.tasks_event.append(TasksEvent(task_id=row["task_id"],
                                               event_id=row["event_id"],
                                               level=row["level"],
                                               question=["question"]))

    def _create_tasks_person(self, path):
        data = pandas.read_csv(os.path.join(path, "tasks_person.csv"))
        self.tasks_person = []
        for index, row in data.iterrows():
            self.tasks_person.append(
                TasksPerson(task_id=row["task_id"],
    ...
    def _create_tasks_result(self, path):
        ...
        for index, row in data.iterrows():
            self.tasks_result.append(
                TasksResult(task_id=row["task_id"],
                            result_id=["result_id"],
                            level=row["level"]))
    ...
    def _create_tasks_reason(self, path):
        ...
        for index, row in data.iterrows():
            self.tasks_reason.append(
                TasksReason(task_id=row["task_id"],
