...
class KnowledgeDatabase(object):
    ...
    def _create_events(self, path):
        ...
        for index, row in data.iterrows():
            self.events.append(Event(event_id=row["event_id"],
    ...
    def _create_persons(self, path):
        ...
        persons = []
        for index, row in data.iterrows():
            self.persons.append(Person(person_id=row["person_id"],
                                       name=row["name"],
    ...
    def _create_terms(self, path):
        ...
        terms = []
        for index, row in data.iterrows():
            self.terms.append(Term(term_id=row["term_id"], term=row["term"], definition=row["definition"]))

    def _create_reasons(self, path):
        data = pandas.read_csv(path + "/reasons.csv")
        reasons = []
        for index, row in data.iterrows():
            self.reasons.append(Reason(reason_id=row["reason_id"], event_id=row["event_id"], description=row["description"]))

    def _create_results(self, path):
        data = pandas.read_csv(path + "/results.csv")
        results = []
        for index, row in data.iterrows():
            self.results.append(Result(result_id=row["reason_id"], event_id=row["event_id"], description=row["description"]))

    def _create_tasks_date(self, path):
        data = pandas.read_csv(path + "/tasks_date.csv")
        tasks_date = []
        for index, row in data.iterrows():
            self.tasks_date.append(TasksDate(task_id=row["task_id"], event_id=row["event_id"], level=row["level"], question=["question"]))

    def _create_tasks_event(self, path):
        data = pandas.read_csv(path + "/tasks_event.csv")
        tasks_event = []
        for index, row in data.iterrows():
            self.tasks_event.append(TasksEvent(task_id=row["task_id"], event_id=row["event_id"], level=row["level"], question=["question"]))
